...
int personDetector_init(const DetectorConfig_t* config)
{
    ...
    // 计算跳帧数（源帧率由 setSourceFps 设置，默认30）
    g_detector.sourceFps = 30;
    g_detector.frameSkip = (g_detector.sourceFps > g_detector.detectFps) ? 
        g_detector.sourceFps / g_detector.detectFps : 1;
...
void personDetector_setFps(int fps)
{
    if (fps > 0 && fps <= 30) {
        ...
        g_detector.frameSkip = (g_detector.sourceFps > fps) ? g_detector.sourceFps / fps : 1;
        g_detector.frameCounter = 0;
        printf("[PersonDetector] FPS set to %d, frameSkip=%d\n", fps, g_detector.frameSkip);
    }
}

void personDetector_setSourceFps(int fps)
{
    if (fps > 0 && fps <= 60) {
        g_detector.sourceFps = fps;
        g_detector.frameSkip = (fps > g_detector.detectFps) ? fps / g_detector.detectFps : 1;
        g_detector.frameCounter = 0;
        printf("[PersonDetector] Source FPS set to %d, frameSkip=%d\n", fps, g_detector.frameSkip);
    }
}
